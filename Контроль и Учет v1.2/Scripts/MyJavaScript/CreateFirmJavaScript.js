    ...
    $(document).ready(
        () => {
            ...
            $('#CreateFirm').click(function () {
                ...
                if ($('#AddressType1').val() == -1) {
                    requestObject.AddressType1 = +$('#AddressType1').val()
                }

                $("#myDiv").html("");

                var select1 = $("#AddressType1");

                if (select1.value == -1) { $('#myDiv').append(`<h4>Невыбран тип адреса для Адрес фирмы №1 !!! </h4>`) }

               */

                $.post("http://localhost:49467/Firm/Create", {
